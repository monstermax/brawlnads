...
contract MonanimalNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    function _getColors(
        ...
        if (keccak256(bytes(scheme)) == keccak256(bytes("purple-blue"))) {
            return
                '<linearGradient id="bg"><stop offset="0%" stop-color="#836EF9"/><stop offset="100%" stop-color="#200052"/></linearGradient><linearGradient id="body"><stop offset="0%" stop-color="#836EF9"/><stop offset="100%" stop-color="#A0055D"/></linearGradient>';
        }
        // Autres schémas de couleurs...
        return
            '<linearGradient id="bg"><stop offset="0%" stop-color="#836EF9"/><stop offset="100%" stop-color="#FBFAF9"/></linearGradient><linearGradient id="body"><stop offset="0%" stop-color="#836EF9"/><stop offset="100%" stop-color="#200052"/></linearGradient>';
    }

    function _getClassShape(
        MonanimalClass class
    ) private pure returns (string memory) {
        if (class == MonanimalClass.Warrior) {
            return
                '<rect x="150" y="150" width="100" height="100" fill="url(#body)" rx="20"/>';
        } else if (class == MonanimalClass.Mage) {
    ...
    function _generateStats(
        ...
        return
            string(
                abi.encodePacked(
                    '<text x="20" y="350" fill="white" font-size="12">Level: ',
                    monanimal.level.toString(),
                    "</text>",
                    '<text x="20" y="365" fill="white" font-size="10">HP: ',
                    monanimal.stats.health.toString(),
                    "</text>",
                    '<text x="20" y="380" fill="white" font-size="10">ATK: ',
                    monanimal.stats.attack.toString(),
                    "</text>",
                    '<text x="120" y="365" fill="white" font-size="10">DEF: ',
                    monanimal.stats.defense.toString(),
                    "</text>",
                    '<text x="120" y="380" fill="white" font-size="10">SPD: ',
