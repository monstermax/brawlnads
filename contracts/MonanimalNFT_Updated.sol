...
contract MonanimalNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        MonanimalSVGGenerator.SVGParams memory svgParams = MonanimalSVGGenerator.SVGParams({
            name: monanimal.name,
    ...
    function _generateMetadata(uint256 tokenId, Monanimal memory monanimal, string memory svg) private pure returns (string memory) {
        return string(abi.encodePacked(
            ...
            '"external_url":"https://brawlnads.monad.xyz/monanimal/', tokenId.toString(), '",',
            '"attributes":[',
            ...
            '{"trait_type":"Color Scheme","value":"', monanimal.colorScheme, '"},',
            '{"trait_type":"Wins","value":', monanimal.wins.toString(), '},',
            '{"trait_type":"Losses","value":', monanimal.losses.toString(), '},',
            '{"trait_type":"Win Rate","value":"', _calculateWinRate(monanimal.wins, monanimal.losses), '%"},',
            '{"trait_type":"Status","value":"', monanimal.isKO ? "KO" : "Ready", '"}',
            ']}'
        ));
    }

    function _calculateWinRate(uint256 wins, uint256 losses) private pure returns (string memory) {
